...
export const RagTroubleshooter: React.FC = () => {
  ...
  const getDiagnosis = () => {
    ...
    return {
      ...
      solution: "Add automated regression tests with DeepEval in GitHub Actions to lock in this accuracy for future model upgrades.",
    ...
    <div className="card" style={{ padding: '2rem', background: 'var(--bg-secondary)', border: '1px solid var(--border-default)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)' }}>
              Pinpoint in 3 clicks whether your RAG hallucination is a Chunking, Embedding, Ranking, or Generator Prompt issue.
            </p>
          </div>
        </div>
      ...
      {!diagnosis ? (
        <div style={{ padding: '1.5rem', background: 'var(--bg-primary)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
          {step === 1 && (
            <div>
              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--accent-cyan)', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
                Step 1: Check Retrieved Context Chunks
              </div>
              <h4 style={{ fontSize: 'var(--text-base)', marginBottom: '1rem', color: 'var(--text-primary)' }}>
          ...
          {step === 2 && (
            <div>
              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--accent-purple)', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
          ...
          {step === 3 && (
            <div>
              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--accent-emerald)', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
        ...
        <div style={{ padding: '1.5rem', background: 'var(--bg-primary)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.6rem', borderRadius: 'var(--radius-full)', background: 'var(--accent-rose-subtle)', color: 'var(--accent-rose)' }}>
              DIAGNOSIS
            </span>
          ...
          <div style={{ background: 'var(--bg-secondary)', padding: '1rem', borderRadius: 'var(--radius-md)', borderLeft: '3px solid var(--accent-cyan)', marginBottom: '1rem' }}>
            <strong style={{ fontSize: 'var(--text-xs)', textTransform: 'uppercase', color: 'var(--accent-cyan)', display: 'block', marginBottom: '0.25rem' }}>
              Recommended QE Fix:
            </strong>
            <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-primary)', margin: 0, lineHeight: 1.6 }}>
              {diagnosis.solution}
            </p>
          </div>

          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
            <strong>Metric to evaluate:</strong> <span style={{ color: 'var(--accent-emerald)', fontFamily: 'var(--font-mono)' }}>{diagnosis.metricToTrack}</span>
          </div>
        </div>
      )}
    </div>
